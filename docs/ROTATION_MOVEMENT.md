...
`provider.settings` является блоком поведенческих настроек специализации, а не продолжением списка способностей. `SpecProvider` автоматически отделяет его в UI заголовком `Поведение ротации`; поэтому `useMovementPriority`, выбор проклятия и будущие аналогичные параметры не должны объявляться как ability-категории только ради отображения в настройках.
