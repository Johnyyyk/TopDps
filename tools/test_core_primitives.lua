...
UnitExists = function() return 1 end
...
local function TestLegacyCasting()
    UnitCastingInfo = function() return "Legacy Cast", "Rank", "Display", "legacy-icon", 100000, 102000, false, 42, true end
...
local function TestLegacyChannelEightReturns()
    ...
    UnitChannelInfo = function() return "Legacy Channel", "Rank", "Display", "legacy-channel-icon", 100000, 103000, false, true end
    local state = CastService:GetPlayerCastState()
...
local function TestLegacyChannelPrivateNineReturns()
    UnitChannelInfo = function() return "Private Channel", "Rank", "Display", "private-icon", 100000, 103000, false, 77, true end
    local state = CastService:GetPlayerCastState()
...
local function TestModernCastingLayouts()
    UnitChannelInfo = function() return nil end
    UnitCastingInfo = function() return "Modern Cast", "Text", "modern-icon", 100000, 102000, false, 55, true, 12345 end
    local state = CastService:GetPlayerCastState()
    ...
    UnitCastingInfo = function() return "Modern Cast Short", "Text", "modern-icon", 100000, 102000, false, 56, 12346 end
...
local function TestModernChannelLayouts()
    UnitCastingInfo = function() return nil end
    UnitChannelInfo = function() return "Modern Channel", "Text", "modern-channel-icon", 100000, 103000, false, true, 22345 end
    local state = CastService:GetPlayerCastState()
    ...
    UnitChannelInfo = function() return "Modern Channel Short", "Text", "modern-channel-icon", 100000, 103000, false, 22346 end
    state = CastService:GetPlayerCastState()
...
local function TestChannelTicks()
    UnitChannelInfo = function() return "Tick Channel", "Rank", "Display", "icon", 100000, 103000, false, false end
    now = 101.1
    local state = CastService:GetPlayerCastState()
...
local mainSpeed = 3
local offSpeed = 2
UnitGUID = function(unit) if unit == "player" then return playerGuid end return nil end
UnitAttackSpeed = function() return mainSpeed, offSpeed end
IsCurrentAction = function(action) return action == 7 and 1 or nil end
TopDps.SpecManager = {
    GetActive = function()
        return {
            abilities = { queued = { swingReset = "MAIN_HAND" } },
            GetSpellCategory = function(_, spellId) if spellId == 5000 then return "queued" end return nil end,
        }
    end,
}
...
local function RecordUnknownSwing()
    SwingService:RecordCombatEvent(now, "SWING_DAMAGE", playerGuid, "Player", 0, "Target-1", "Target", 0, 100, 0, 1, 0, 0, 0, false, false, false)
end
...
local function TestExplicitOffHand()
    SwingService:Clear()
    now = 2
    SwingService:RecordCombatEvent(now, "SWING_DAMAGE", playerGuid, "Player", 0, "Target-1", "Target", 0, 100, 0, 1, 0, 0, 0, false, false, false, true)
...
local function TestAbilitySwingReset()
    ...
    offSpeed = 0
    now = 5
    SwingService:RecordCombatEvent(now, "SPELL_DAMAGE", playerGuid, "Player", 0, "Target-1", "Target", 0, 5000, "Queued Strike", 1, 100, 0, 1, 0, 0, 0, false, false, false)
...
GetSpellInfo = function(spellId)
    if spellId == 50842 then return "Pestilence" end
...
local function TestPlayerCastTracking()
    CombatTracker:Clear()
    now = 20
    CombatTracker:RecordCombatEvent(now, "SPELL_CAST_SUCCESS", playerGuid, "Player", 0, "Target-1", "Target", 0, 50842, "Pestilence", 1)
    ...
    now = 21
    CombatTracker:RecordCombatEvent(now, "SPELL_CAST_SUCCESS", playerGuid, "Player", 0, "Target-1", "Target", 0, 99999, "Pestilence", 1)
